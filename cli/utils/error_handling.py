...
from utils.sentry_config import log_unexpected_error
...
def handle_cli_errors(func: Callable) -> Callable:
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            click.echo(click.style(f"Access denied: {e}", fg="red", bold=True))
            click.echo(
                click.style(
                    "Tip: Check that you have the necessary"
            ...
            click.echo(click.style(f" Invalid data: {e}", fg="red", bold=True))
            click.echo(
                click.style(
            ...
            if "email" in str(e).lower():
                click.echo(
                    click.style("This email address is already in use.", fg="yellow")
                )
            elif "employee_number" in str(e).lower():
                click.echo(
                    click.style("This employee number already exists.", fg="yellow")
                )
            else:
                click.echo(
                    click.style(
                        "Check that the data doesn't already exist.", fg="yellow"
                    )
                )
            ...
            click.echo(
                click.style(
                    "Check that the database is accessible and"
            ...
            click.echo(
                click.style(f"Resource not found: {e.message}", fg="red", bold=True)
            )
            click.echo(
                click.style(
                    "Check the ID or use the 'list' command"
            ...
            click.echo(click.style(f"Business error: {e.message}", fg="red", bold=True))
            click.echo(
                click.style(
                    "This action is not allowed in the current context.", fg="yellow"
                )
            )
            ...
            click.echo(
                click.style(f"Database connection: {e.message}", fg="red", bold=True)
            )
            click.echo(
                click.style("Check connection settings in config.py", fg="yellow")
            )
            logger.error(f"Database connection error in {func.__name__}: {e}")
            raise click.Abort()
            ...
            click.echo(
                click.style("Operation cancelled by user.", fg="yellow", bold=True)
            )
            raise click.Abort()

        except Exception as e:
            # Log l'erreur inattendue dans Sentry avec contexte
            context = {
    ...
    try:
        ...
        if num <= 0:
            raise ValidationError(
                f"{field_name} must be a positive number (received: {value})"
            )
        return num
    except (ValueError, TypeError):
        raise ValidationError(
            f"{field_name} must be a valid number (received: {value})"
        )
...
def validate_id(value: Any, resource_name: str = "ID") -> int:
    ...
    try:
        ...
        if id_val <= 0:
            raise ValidationError(
                f"{resource_name} must be a positive integer (received: {value})"
            )
        return id_val
    except (ValueError, TypeError):
        raise ValidationError(
            f"{resource_name} must be a valid integer (received: {value})"
        )
...
def validate_date_format(date_str: str, field_name: str = "Date") -> str:
    ...
    if not re.match(r"^\d{4}-\d{2}-\d{2}$", date_str):
        raise ValidationError(
            f"{field_name} must be in YYYY-MM-DD format (received: {date_str})"
        )
    ...
    try:
        ...
        raise ValidationError(
            f"{field_name} is not a valid date (received: {date_str})"
        )
...
def confirm_destructive_action(action_description: str) -> bool:
    ...
    """
    click.echo(
        click.style(f"Destructive action: {action_description}", fg="yellow", bold=True)
    )
    return click.confirm(
        click.style("Are you sure you want to continue?", fg="red"), default=False
    )
