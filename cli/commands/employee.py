...
def list_employees(role, limit):
    ...
    try:
        ...
        for emp in employees:
            click.echo(f"{emp.id:<5} {emp.name:<25} {emp.email:<30} {emp.role or 'N/A':<15} {emp.employee_number:<10}")
...
@click.option('--role-id', prompt='Role ID (1=sales, 2=support, 3=management)', 
              type=int, help='Employee role ID')
@click.option('--password', prompt='Password', hide_input=True, 
              confirmation_prompt=True, help='Employee password')
@cli_auth_required
...
def update_employee(employee_id, name, email, role_id):
    ...
    click.echo(click.style(f'=== Update Employee {employee_id} ===', fg='blue', bold=True))
    ...
    try:
        ...
        if not any([name, email, role_id]):
            ...
            new_name = click.prompt('Name', default=existing_employee.name, show_default=True)
            new_email = click.prompt('Email', default=existing_employee.email, show_default=True)
            new_role_id = click.prompt('Role ID', default=existing_employee.role_id, 
        ...
        updated_employee = service.update_employee(employee_id, update_data, current_user)
...
def delete_employee(employee_id, force):
    ...
    try:
        ...
        existing_employee = service.get_employee(employee_id)
        if not existing_employee:
            raise ResourceNotFoundError(f"Employee with ID {employee_id} does not exist.")
        
        # Show employee details
        click.echo(click.style(f'=== Delete Employee {employee_id} ===', fg='red', bold=True))
        ...
        # Confirmation
        if not force:
            if not confirm_destructive_action(f"delete employee '{existing_employee.full_name}'"):
        ...
        display_success_message(f"Employee '{existing_employee.full_name}' deleted successfully!")
