...
def list_customers(sales_contact, limit):
    ...
    try:
        ...
        if sales_contact:
            customers = [
                cust for cust in customers if cust.sales_contact_id == sales_contact
            ]
...
@click.option(
    "--contact-name", prompt="Contact name", help="Customer contact person name"
)
...
def update_customer(
    ...
    click.echo(
        click.style(f"=== Update Customer {customer_id} ===", fg="blue", bold=True)
    )
    ...
    try:
        ...
        if not any(
            ...
            new_company = click.prompt(
                "Company name",
                default=existing_customer.company_name,
                show_default=True,
            )
            new_contact = click.prompt(
                "Contact name", default=existing_customer.full_name, show_default=True
            )
            new_email = click.prompt(
                "Email", default=existing_customer.email, show_default=True
            )
            ...
            update_data.update(
                {
                    ...
                    "sales_contact_id": (
                        validate_id(new_sales_id, "Sales contact ID")
                        if new_sales_id
                        else None
                    ),
                }
...
def delete_customer(customer_id, force):
    ...
    try:
        ...
        click.echo(
            click.style(f"=== Delete Customer {customer_id} ===", fg="red", bold=True)
        )
        ...
        if not force:
            if not confirm_destructive_action(
                f"delete customer '{existing_customer.company_name}'"
            ):
        ...
        display_success_message(
            f"Customer '{existing_customer.company_name}' deleted successfully!"
        )
