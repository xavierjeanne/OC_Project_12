...
Focus sur les lignes manquantes facilement testables identifiées dans le rapport.
"""
import pytest
...
from utils.validators import (
    validate_date, validate_phone, ValidationError,
    validate_string_not_empty, validate_email,
    validate_positive_amount, validate_role
)
from utils.permissions import (
    Permission, PermissionError, require_permission,
    has_permission
)


class TestValidatorsCoverage:
    """Tests pour améliorer la couverture des validators (actuellement 94%)"""
    
    def test_validate_date_invalid_format(self):
        """Test validation date avec format invalide - ligne 86"""
        with pytest.raises(ValidationError):
            validate_date("invalid-date", "test_date")
    
    def test_validate_phone_invalid_format(self):
        """Test validation téléphone avec format invalide - lignes 114, 118-119"""
...
class TestPermissionsCoverage:
    """Tests pour améliorer la couverture des permissions (actuellement 96%)"""
    
    def test_permission_enum_values(self):
        """Test des valeurs de l'enum Permission - ligne 205"""
...
class TestDatabaseConfigCoverage:
    """Tests pour améliorer la couverture de db.config (actuellement 62%)"""
    ...
    def test_database_url_construction(self):
        """Test construction de l'URL de database"""
        import db.config
        # Force le rechargement du module pour prendre en compte les nouvelles variables
        import importlib
        importlib.reload(db.config)
        
        assert 'postgresql+psycopg2://test_user:test_password@localhost:5432/test_db' in db.config.DATABASE_URL
    
    @patch('db.config.engine')
    def test_connection_test_success(self, mock_engine):
        """Test de la fonction test_connection avec succès"""
        ...
        # Ceci devrait couvrir les lignes dans test_connection
        test_connection()
    
    @patch('db.config.engine')
    def test_connection_test_failure(self, mock_engine):
        """Test de la fonction test_connection avec échec"""
        from db.config import test_connection
        
        mock_engine.connect.side_effect = Exception("Connection failed")
        
        # Ceci devrait couvrir la branche d'exception
        test_connection()


class TestModelsAttributesCoverage:
    """Tests pour améliorer la couverture des modèles avec __repr__ non testés"""
    
    def test_employee_repr(self):
        """Test représentation string Employee - ligne 43"""
    ...
    def test_customer_repr(self):
        """Test représentation string Customer - lignes 32-34"""
    ...
    def test_contract_repr(self):
        """Test représentation string Contract - lignes 29-31"""
    ...
    def test_event_repr(self):
        """Test représentation string Event - lignes 32-34"""
    ...
    def test_role_repr(self):
        """Test représentation string Role - ligne 21"""
...
class TestBaseModelCoverage:
    """Tests pour améliorer la couverture de models.base"""
    
    def test_base_init_db(self):
        """Test fonction init_db - lignes 17-18"""
        from models.base import init_db
        # Test de la fonction init_db (sans réellement créer les tables)
...
class TestJWTServiceCoverage:
    """Tests pour améliorer la couverture de jwt_service"""
    
    def test_jwt_service_initialization(self):
        """Test initialisation JWT service"""
...
class TestAuthManagerCoverage:
    """Tests pour améliorer la couverture de auth_manager"""
    
    def test_auth_manager_initialization(self):
        """Test initialisation auth manager"""
